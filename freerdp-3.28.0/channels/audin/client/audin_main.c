                                    ...
                                    void* user_data)
{
	...
	WLog_Print(audin->log, WLOG_ERROR,
	           "[audin_trace] audin_receive_wave_data: entered, size=%zu attached=%d "
	           "device_fmt=%dx%du%uHz audin_fmt=0x%04x",
	           size, audin->attached, format->nChannels, format->wBitsPerSample,
	           format->nSamplesPerSec, audin->format ? audin->format->wFormatTag : 0);
	...
	{
		WLog_Print(audin->log, WLOG_ERROR,
		           "[audin_trace] audin_receive_wave_data: EnsureRemainingCapacity FAILED");
		return CHANNEL_RC_NO_MEMORY;
	}
	...
	WLog_Print(audin->log, WLOG_ERROR,
	           "[audin_trace] audin_receive_wave_data: compatible=%d", compatible);

	if (compatible && audin->device->FormatSupported(audin->device, audin->format))
	{
		WLog_Print(audin->log, WLOG_ERROR,
		           "[audin_trace] audin_receive_wave_data: direct write path, size=%zu", size);
	...
	{
		WLog_Print(audin->log, WLOG_ERROR,
		           "[audin_trace] audin_receive_wave_data: calling freerdp_dsp_encode, "
		           "dsp=%p fmt_chan=%u fmt_rate=%u fmt_bits=%u",
		           (void*)audin->dsp_context, format->nChannels, format->nSamplesPerSec,
		           format->wBitsPerSample);
		if (!freerdp_dsp_encode(audin->dsp_context, format, data, size, audin->data))
		{
			WLog_Print(audin->log, WLOG_ERROR,
			           "[audin_trace] audin_receive_wave_data: freerdp_dsp_encode FAILED");
			return ERROR_INTERNAL_ERROR;
		}
		WLog_Print(audin->log, WLOG_ERROR,
		           "[audin_trace] audin_receive_wave_data: freerdp_dsp_encode OK");
	}
	...
	{
		WLog_Print(audin->log, WLOG_ERROR,
		           "[audin_trace] audin_receive_wave_data: no data produced, skip");
		return CHANNEL_RC_OK;
	}
	...
	WLog_Print(audin->log, WLOG_ERROR,
	           "[audin_trace] audin_receive_wave_data: sending data pdu, pos=%" PRIuz,
	           Stream_GetPosition(audin->data));

	if ((error = audin_send_incoming_data_pdu(callback)))
	{
		WLog_Print(audin->log, WLOG_ERROR,
		           "[audin_trace] audin_receive_wave_data: audin_send_incoming_data_pdu failed "
		           "with error %" PRIu32,
		           error);
		return error;
	}
	...
	WLog_Print(audin->log, WLOG_ERROR,
	           "[audin_trace] audin_receive_wave_data: calling channel_write_and_free");
	error = audin_channel_write_and_free(callback, audin->data, FALSE);
	WLog_Print(audin->log, WLOG_ERROR,
	           "[audin_trace] audin_receive_wave_data: channel_write_and_free returned %" PRIu32,
	           error);
	return error;
}
...
static BOOL audin_open_device(AUDIN_PLUGIN* audin, AUDIN_CHANNEL_CALLBACK* callback)
{
	...
	WLog_Print(audin->log, WLOG_ERROR, "[audin_trace] audin_open_device: entered, device=%p",
	           (void*)audin->device);
	...
	WLog_Print(audin->log, WLOG_ERROR, "[audin_trace] audin_open_device: FormatSupported=%d",
	           supported);
	...
	if (!supported)
	{
		...
		{
			WLog_Print(audin->log, WLOG_ERROR,
			           "[audin_trace] audin_open_device: no supported format found");
			return FALSE;
		}
		WLog_Print(audin->log, WLOG_ERROR,
		           "[audin_trace] audin_open_device: fallback format %" PRIu32 "x%u %uHz",
	...
	WLog_Print(audin->log, WLOG_ERROR, "[audin_trace] audin_open_device: SetFormat OK");

	/* 采集格式与服务器格式一致时，audin_receive_wave_data 走 direct write 直通，
	 * 不需要 dsp 编码器。某些构建（如 .deps 精简 FFmpeg 无 PCM 编码器）下
	 * dsp_context_reset 会失败，此时不应阻塞 open（PCM 直通即可正常工作）。 */
	if (audio_format_compatible(&format, audin->format) &&
	    IFCALLRESULT(FALSE, audin->device->FormatSupported, audin->device, audin->format))
	{
		WLog_Print(audin->log, WLOG_ERROR,
		           "[audin_trace] audin_open_device: format compatible, direct path without dsp");
	}
	else if (!freerdp_dsp_context_reset(audin->dsp_context, audin->format, audin->FramesPerPacket))
	{
		WLog_Print(audin->log, WLOG_ERROR, "[audin_trace] audin_open_device: dsp_context_reset FAILED");
		return FALSE;
	}
	WLog_Print(audin->log, WLOG_ERROR, "[audin_trace] audin_open_device: calling device->Open");
	...
	WLog_Print(audin->log, WLOG_ERROR, "[audin_trace] audin_open_device: device->Open SUCCESS");
	return TRUE;
}
...
static UINT audin_process_open(AUDIN_PLUGIN* audin, AUDIN_CHANNEL_CALLBACK* callback, wStream* s)
{
	...
	WLog_Print(audin->log, WLOG_ERROR, "[audin_trace] audin_process_open: entered");
	...
	WLog_Print(audin->log, WLOG_ERROR, "[audin_trace] audin_process_open: calling audin_open_device");

	if (!audin_open_device(audin, callback))
		return ERROR_INTERNAL_ERROR;

	WLog_Print(audin->log, WLOG_ERROR, "[audin_trace] audin_process_open: audin_open_device SUCCESS");
...
static UINT audin_on_data_received(IWTSVirtualChannelCallback* pChannelCallback, wStream* data)
{
	...
	WLog_Print(audin->log, WLOG_ERROR, "[audin_trace] audin_on_data_received: MessageId=0x%02" PRIx8 "",
	           MessageId);
...
FREERDP_ENTRY_POINT(UINT VCAPITYPE audin_DVCPluginEntry(IDRDYNVC_ENTRY_POINTS* pEntryPoints))
{
	...
	WLog_Print(WLog_Get(TAG), WLOG_ERROR, "[audin_trace] audin_DVCPluginEntry: creating plugin");
	...
	WLog_Print(audin->log, WLOG_ERROR, "[audin_trace] audin_DVCPluginEntry: registering plugin with device=%p",
	           (void*)audin->device);
	error = pEntryPoints->RegisterPlugin(pEntryPoints, AUDIN_CHANNEL_NAME, &audin->iface);
	if (error == CHANNEL_RC_OK)
	{
		WLog_Print(audin->log, WLOG_ERROR, "[audin_trace] audin_DVCPluginEntry: plugin registered OK");
		return error;
	}
