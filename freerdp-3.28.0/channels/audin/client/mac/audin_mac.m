...
static BOOL audin_mac_format_supported(IAudinDevice *device, const AUDIO_FORMAT *format)
{
	...
	/* AudioQueue handles 1 or 2 channels natively; Windows servers may
	 * negotiate mono formats, so do not hard-require stereo. */
 ...
 * FALSE and the device open failed. Block (up to 30s) so that by the time
 * the channel negotiates formats / opens the device the state is final.
 *
 * @return TRUE if microphone access is granted
 */
static BOOL audin_mac_ensure_authorization(void)
{
#if defined(MAC_OS_X_VERSION_10_14)
	if (!@available(macOS 10.14, *))
		return TRUE; /* pre-10.14 has no TCC microphone prompt */
    ...
    PFREERDP_AUDIN_DEVICE_ENTRY_POINTS pEntryPoints))
{
	...
	@autoreleasepool
	{
		mac->isAuthorized = audin_mac_ensure_authorization();
		if (!mac->isAuthorized)
			WLog_WARN(TAG, "Microphone access not granted (denied/restricted)");
	}
