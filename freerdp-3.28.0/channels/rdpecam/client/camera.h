...
/* Idle auto-stop: if the server stops sending SampleRequest for this long
 * (e.g. the camera app on the remote side was closed), pause the capture so
 * the camera hardware (and its activity LED) is released even when the server
 * never sends an explicit StopStreamsRequest / DeactivateDeviceRequest.
 * A later SampleRequest transparently resumes the stream. */
#define ECAM_STREAM_IDLE_TIMEOUT_MS 2000u
#define ECAM_IDLE_CHECK_INTERVAL_MS 500u
...
typedef struct
{
	...
	/* Idle auto-stop timer (see ECAM_STREAM_IDLE_TIMEOUT_MS). */
	HANDLE timerQueue;
	HANDLE idleTimer;
	CRITICAL_SECTION devicesLock; /**< guards the devices hash table vs timer walk */
...
typedef struct
{
	...
	/* MF H.264 encoder state (Windows only, NULL if not used) */
	void* mfEncoder;
	HANDLE mfEncoderThread;
	BOOL mfEncoderActive;
	BYTE* mfEncoderOutput;
	UINT32 mfEncoderOutputSize;
	UINT32 mfEncoderOutputCap;

	/* Idle auto-stop state (see ECAM_STREAM_IDLE_TIMEOUT_MS). */
	UINT64 lastSampleRequestTime; /**< GetTickCount64 of the last SampleRequest */
	BOOL autoPaused;              /**< TRUE when idle timeout paused the capture */
} CameraDeviceStream;
...
typedef struct
{
	...
	char channelName[64]; /**< DVC name sent in DeviceAddedNotification */
...
/* Idle auto-stop: called from the periodic idle-check timer. Pauses the capture
 * (releases the camera hardware) when no SampleRequest arrived for a while.
 * Returns TRUE if the stream was paused. */
WINPR_ATTR_NODISCARD FREERDP_LOCAL BOOL ecam_dev_pause_stream_idle(CameraDevice* dev,
                                                                   size_t streamIndex,
                                                                   UINT64 now);
