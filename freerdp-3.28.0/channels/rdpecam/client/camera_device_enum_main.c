                                            ...
                                            const char* deviceId, const char* deviceName)
{
	...
	/* Guard the devices hash table against the concurrent idle-check timer walk. */
	EnterCriticalSection(&ecam->devicesLock);

	/* If this camera was already registered during a previous enumeration
	 * cycle, keep the existing CameraDevice (don't destroy/re-create it).
	 * But DO resend the DeviceAddedNotification so the server knows the
	 * device is still available — necessary when the server deactivates
	 * the device (without ever starting a stream) and the enumeration
	 * channel reconnects. */
	if (HashTable_ContainsKey(ecam->devices, deviceId))
	{
		CameraDevice* dev = (CameraDevice*)HashTable_GetItemValue(
		    ecam->devices, deviceId);
		if (dev)
		{
			WLog_DBG(TAG,
			         "Device %s already registered, re-notifying (channelName=%s)",
			         deviceId, dev->channelName);
			ecam_send_device_added_notification(ecam, hchannel, deviceName,
			                                    dev->channelName);
		}
		LeaveCriticalSection(&ecam->devicesLock);
		return CHANNEL_RC_OK;
	}

	/* Generate a clean, unique channel name for this camera device.
	 * Windows RDP server expects a simple DVC name (e.g. "RDCamera_0"),
	 * NOT the raw deviceId (symbolic link) which contains characters
	 * like \, #, & that the server cannot handle. */
	char channelName[32] = {};
	const size_t deviceIndex = HashTable_Count(ecam->devices);
	snprintf(channelName, sizeof(channelName), "RDCamera_%zu", deviceIndex);
	...
	WLog_INFO(TAG, "Sent DeviceAddedNotification: deviceName=%s, channelName=%s", deviceName,
	          channelName);
...
/**
 * Periodic idle check (see ECAM_STREAM_IDLE_TIMEOUT_MS). Pauses any stream
 * that has not received a SampleRequest for a while, releasing the camera
 * hardware (e.g. turns the activity LED off) even if the server never sends
 * an explicit StopStreamsRequest / DeactivateDeviceRequest.
 */
static VOID CALLBACK ecam_dev_idle_check_timer(PVOID param, BOOLEAN timerOrWaitFired)
{
	CameraPlugin* ecam = (CameraPlugin*)param;
	WINPR_UNUSED(timerOrWaitFired);

	if (!ecam || !ecam->devices)
		return;
	...
	const size_t count = HashTable_GetKeys(ecam->devices, &keys);
	for (size_t i = 0; i < count; i++)
	{
		CameraDevice* dev =
		    (CameraDevice*)HashTable_GetItemValue(ecam->devices, (const void*)keys[i]);
		if (!dev)
			continue;

		for (size_t si = 0; si < ECAM_DEVICE_MAX_STREAMS; si++)
			ecam_dev_pause_stream_idle(dev, si, now);
	}
	free(keys);
...
static UINT ecam_plugin_initialize(IWTSPlugin* pPlugin, IWTSVirtualChannelManager* pChannelMgr)
{
	...
	/* Periodic idle check: auto-pause streams whose server stopped requesting. */
	ecam->timerQueue = CreateTimerQueue();
	if (ecam->timerQueue &&
	    CreateTimerQueueTimer(&ecam->idleTimer, ecam->timerQueue, ecam_dev_idle_check_timer, ecam,
	                          ECAM_IDLE_CHECK_INTERVAL_MS, ECAM_IDLE_CHECK_INTERVAL_MS, 0))
	{
		WLog_DBG(TAG, "Idle auto-stop timer started (timeout=%u ms, check interval=%u ms)",
		         ECAM_STREAM_IDLE_TIMEOUT_MS, ECAM_IDLE_CHECK_INTERVAL_MS);
	}
	else
	{
		WLog_ERR(TAG, "Unable to create idle auto-stop timer");
		ecam->timerQueue = nullptr;
		ecam->idleTimer = nullptr;
	}
...
static UINT ecam_plugin_terminated(IWTSPlugin* pPlugin)
{
	...
	/* Stop the idle check timer before tearing down the devices table. */
	if (ecam->idleTimer)
	{
		DeleteTimerQueueTimer(ecam->timerQueue, ecam->idleTimer, nullptr);
		ecam->idleTimer = nullptr;
	}
	if (ecam->timerQueue)
	{
		DeleteTimerQueueEx(ecam->timerQueue, nullptr);
		ecam->timerQueue = nullptr;
	}

	/* Destroy device listeners FIRST, before the enumeration channel manager
	 * is torn down. This ensures the server sees the devices disappear
	 * (via DVC listener removal) while the channel manager is still valid. */
	HashTable_Free(ecam->devices);
	...
	DeleteCriticalSection(&ecam->devicesLock);
...
FREERDP_ENTRY_POINT(UINT VCAPITYPE rdpecam_DVCPluginEntry(IDRDYNVC_ENTRY_POINTS* pEntryPoints))
{
	...
	if (!InitializeCriticalSectionEx(&ecam->devicesLock, 0, 0))
	{
...
#if defined(WITH_V4L)
	ecam->subsystem = "v4l";
#elif defined(WITH_WMF)
	ecam->subsystem = "wmf";
#elif defined(WITH_AVF)
	ecam->subsystem = "avf";
