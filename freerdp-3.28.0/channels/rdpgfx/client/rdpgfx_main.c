 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static DWORD WINAPI rdpgfx_decode_thread_main(LPVOID arg)
{
	RDPGFX_PLUGIN* gfx = (RDPGFX_PLUGIN*)arg;
	wMessage msg;

	WINPR_ASSERT(gfx);
	WINPR_ASSERT(gfx->decodeQueue);

	while (MessageQueue_Get(gfx->decodeQueue, &msg) > 0)
	{
		GENERIC_CHANNEL_CALLBACK* callback = (GENERIC_CHANNEL_CALLBACK*)msg.lParam;
		wStream* data = (wStream*)msg.wParam;
		UINT error = CHANNEL_RC_OK;
		UINT32 DstSize = 0;
		...
		int status = zgfx_decompress(gfx->zgfx, Stream_Buffer(data),
		                             (UINT32)Stream_Length(data), &pDstData, &DstSize, 0);
		...
		if (callback)
		{
			while (Stream_GetPosition(s) < Stream_Length(s))
			{
				...
				{
					WLog_Print(gfx->base.log, WLOG_ERROR,
					           "rdpgfx_recv_pdu failed with error %" PRIu32 "!", error);
					break;
				}
			}
		}
...
static UINT rdpgfx_stop_decode_thread(RDPGFX_PLUGIN* gfx)
{
	WINPR_ASSERT(gfx);

	if (gfx->decodeQueue)
	{
		if (!MessageQueue_PostQuit(gfx->decodeQueue, 0))
			WLog_Print(gfx->base.log, WLOG_ERROR, "MessageQueue_PostQuit failed!");
	}

	if (gfx->decodeThread)
	{
		if (WaitForSingleObject(gfx->decodeThread, INFINITE) != WAIT_OBJECT_0)
			WLog_Print(gfx->base.log, WLOG_ERROR, "WaitForSingleObject on decode thread failed!");
		CloseHandle(gfx->decodeThread);
		gfx->decodeThread = nullptr;
	}

	if (gfx->decodeQueue)
	{
		MessageQueue_Free(gfx->decodeQueue);
		gfx->decodeQueue = nullptr;
	}
...
static UINT rdpgfx_on_data_received(IWTSVirtualChannelCallback* pChannelCallback, wStream* data)
{
	...
	const size_t remaining = Stream_GetRemainingLength(data);
	if (remaining < 1)
		return CHANNEL_RC_OK;

	/* Copy the payload: the receive thread reuses this stream, so the decode
	 * thread must work on its own buffer. */
	wStream* copy = Stream_New(nullptr, remaining);
	if (!copy)
	{
		WLog_Print(gfx->base.log, WLOG_ERROR, "Stream_New failed!");
		return CHANNEL_RC_NO_MEMORY;
	...
	Stream_Write(copy, Stream_ConstPointer(data), remaining);
	Stream_SealLength(copy);

	if (!MessageQueue_Post(gfx->decodeQueue, gfx, 0, (void*)copy, (void*)callback))
	{
		WLog_Print(gfx->base.log, WLOG_ERROR, "MessageQueue_Post failed!");
		Stream_Free(copy, TRUE);
		return CHANNEL_RC_NO_MEMORY;
	}
...
static UINT rdpgfx_on_close(IWTSVirtualChannelCallback* pChannelCallback)
{
	...
	/* Stop the decode worker before freeing the per-channel callback, so it
	 * cannot touch a dangling callback or zgfx context. */
	rdpgfx_stop_decode_thread(gfx);
                           ...
                           WINPR_ATTR_UNUSED rdpSettings* settings)
{
	...
	gfx->decodeQueue = MessageQueue_New(nullptr);
	if (!gfx->decodeQueue)
	{
		WLog_Print(gfx->base.log, WLOG_ERROR, "MessageQueue_New failed!");
	...
	gfx->decodeThread = CreateThread(nullptr, 0, rdpgfx_decode_thread_main, gfx, 0, nullptr);
	if (!gfx->decodeThread)
	{
		WLog_Print(gfx->base.log, WLOG_ERROR, "CreateThread failed!");
		MessageQueue_Free(gfx->decodeQueue);
		gfx->decodeQueue = nullptr;
		zgfx_context_free(gfx->zgfx);
		gfx->zgfx = nullptr;
...
void rdpgfx_client_context_free(RdpgfxClientContext* context)
{
	...
	/* Safety net in case the channel was never opened/closed (no-op if the
	 * decode thread was already stopped in rdpgfx_on_close). */
	rdpgfx_stop_decode_thread(gfx);
