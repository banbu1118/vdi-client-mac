...
typedef struct
{
	...
	/* Decode worker thread: moves zgfx decompression + PDU handling off the
	 * DVC receive thread so it cannot starve other channels (e.g. rdpecam
	 * SampleRequest handling) while the desktop H.264 (AVC444) is decoded. */
	wMessageQueue* decodeQueue;
	HANDLE decodeThread;
} RDPGFX_PLUGIN;
