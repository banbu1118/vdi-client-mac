...
#endif
	AVPacket* packet;
	/* One-frame-delayed pipeline state for asynchronous hardware encoders
	 * (VideoToolbox): set once the first frame has been fed to the encoder. */
	BOOL encoderFed;
...
static BOOL libavcodec_create_encoder_context(H264_CONTEXT* WINPR_RESTRICT h264)
{
	...
	switch (h264->RateControlMode)
	{
		...
		case H264_RATECONTROL_CQP:
			if (av_opt_set_int(sys->codecEncoderContext, "qp", h264->QP, AV_OPT_SEARCH_CHILDREN) <
			    0)
			{
				/* Encoder without a CQP/qp option (e.g. libopenh264): fall back to an
				 * explicit bitrate target, otherwise the encoder silently uses its
				 * internal defaults and quality cannot be controlled. */
				WLog_Print(h264->log, WLOG_WARN,
				           "av_opt_set_int(qp) failed, falling back to bitrate %" PRIu32,
				           h264->BitRate);
				sys->codecEncoderContext->bit_rate = h264->BitRate;
			}
			break;
	...
	/* Real-time hint for hardware encoders (VideoToolbox); ignored by encoders
	 * that do not expose the option. */
	av_opt_set_int(sys->codecEncoderContext, "realtime", 1, AV_OPT_SEARCH_CHILDREN);
...
#endif
	{
		av_opt_set(sys->codecEncoderContext, "preset", "ultrafast", AV_OPT_SEARCH_CHILDREN);
                               ...
                               BYTE** WINPR_RESTRICT ppDstData, UINT32* WINPR_RESTRICT pDstSize)
{
...
#endif
	{
		/* VideoToolbox (and similar hardware encoders) encode asynchronously:
		 * avcodec_send_frame() only queues the frame, and the encoded packet is
		 * delivered once the NEXT frame is fed to the encoder (the source frame
		 * data is copied synchronously, so the caller may reuse its buffers).
		 * We therefore run a one-frame-delayed pipeline to keep this function
		 * synchronous:
		 *  - first call: queue frame 0, no packet can be ready yet -> return
		 *    rc=-2 (the caller drops that first output; frame 0 itself is
		 *    delivered by the next call).
		 *  - every later call: queue the current frame, then drain the packet
		 *    of the previously queued frame. A transient EAGAIN (hardware
		 *    encoder busy) drops the sample rather than stalling the stream. */
		if (!sys->encoderFed)
		{
			sys->encoderFed = TRUE;
			...
			rc = -2; /* first output arrives with the next call */
			goto done;
		}
		...
		{
			/* Asynchronous hardware encoder (VideoToolbox) not ready yet:
			 * drop this sample immediately instead of polling. A busy-wait
			 * here stalls the capture thread and delays the SampleRequest
			 * response, which throttles the server's request pacing and
			 * makes the video appear laggy. The frame fed to the encoder in
			 * this call is still queued and comes out with the next call. */
			status = 0;
			rc = -2;
		}
		else
	...
	{
		/* No access unit produced in this call (first warm-up frame or a
		 * transient hardware-encoder underrun): let the caller drop the sample
...
static BOOL libavcodec_init(H264_CONTEXT* h264)
{
	...
	else
	{
		...
		if (!sys->codecEncoder)
		{
			/* Prefer the hardware VideoToolbox encoder on macOS: software H.264
			 * encoding (libopenh264 etc.) is too slow for real-time camera streams.
			 * h264_videotoolbox is usually not the first AV_CODEC_ID_H264 encoder
			 * registered, so look it up by name before falling back. */
			sys->codecEncoder = avcodec_find_encoder_by_name("h264_videotoolbox");
			if (sys->codecEncoder)
				WLog_Print(h264->log, WLOG_INFO,
				           "Using VideoToolbox for accelerated H264 encoding");
		}
		if (!sys->codecEncoder)
		{
