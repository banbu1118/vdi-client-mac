...
static UINT32 video_get_h264_bitrate(wLog* log, UINT32 height)
{
	static struct
	{
		...
		/* 摄像头画面运动较多，码率给足避免模糊（当前仅摄像头编码走这里） */
		{ 1080, 4500 }, { 720, 2500 }, { 480, 1200 }, { 360, 700 },
		{ 240, 350 },   { 180, 200 },  { 0, 150 },
	};
                                  ...
                                  FREERDP_VIDEO_FORMAT dstFormat, wStream* output)
{
	...
	if (dstFormat == FREERDP_VIDEO_FORMAT_H264)
	{
		...
		const INT32 hrc = h264_compress(context->h264, &h264Data, &h264Size);
		if (hrc < 0)
		{
			/* rc == -2 means "no access unit produced in this call": the first
			 * warm-up frame(s) of an asynchronous hardware encoder (VideoToolbox)
			 * and transient underruns fall here. That is not an error, just a
			 * dropped sample. */
			if (hrc == -2)
				WLog_Print(context->log, WLOG_DEBUG,
				           "H264 encoder produced no frame (warm-up/underrun)");
			else
