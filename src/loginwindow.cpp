...
// 查找 FreeRDP 可执行文件路径
// 在 macOS app bundle 中搜索 sdl3-freerdp/sfreerdp 二进制文件
QString LoginWindow::findFreeRdpBinary()
{
    QString appDir = QCoreApplication::applicationDirPath();
    qDebug() << "findFreeRdpBinary: appDir =" << appDir;

    // 在 macOS .app bundle 中，appDir = .../VDIClient.app/Contents/MacOS
    // sdl3-freerdp 会被 CMake 复制到 Resources/sdl3-freerdp/
    // 二进制位置: Resources/sdl3-freerdp/Contents/MacOS/{sdl3-freerdp,sfreerdp,sdl-freerdp}
    
    QString resourcesDir = QDir(appDir).absoluteFilePath("../Resources");
    qDebug() << "findFreeRdpBinary: resourcesDir =" << resourcesDir;

    QStringList searchPaths;
    searchPaths << appDir;  // 应用同目录
    searchPaths << resourcesDir;  // Resources 目录
    searchPaths << QDir(appDir).absoluteFilePath("../Frameworks");  // Frameworks 目录

    QStringList binaryNames = {"sdl3-freerdp", "sfreerdp", "sdl-freerdp", "wlfreerdp", "xfreerdp"};
    
    // 搜索各种可能的路径组合（优先搜索深层路径，避免匹配到目录而非文件）
    QStringList pathCombinations;
    for (const QString &basePath : searchPaths) {
        for (const QString &name : binaryNames) {
            // 先搜索 sdl3-freerdp.app 内部的包结构（优先级高）
            pathCombinations << QDir(basePath).absoluteFilePath("sdl3-freerdp.app/Contents/MacOS/" + name);
            // 再搜索裸路径（优先级低，避免目录被 isExecutable 误匹配）
            pathCombinations << QDir(basePath).absoluteFilePath(name);
        }
    }

    for (const QString &path : pathCombinations) {
        QFileInfo fi(path);
        qDebug() << "findFreeRdpBinary: checking" << path;
        if (fi.exists() && fi.isFile() && fi.isExecutable()) {
            qDebug() << "findFreeRdpBinary: found at" << fi.absoluteFilePath();
            return fi.absoluteFilePath();
        }
    }

    qDebug() << "findFreeRdpBinary: not found";
    return QString();
}

// 恢复指定虚拟机的连接按钮状态
void LoginWindow::restoreConnectButtonForVm(const QString &vmId)
{
    if (vmId.isEmpty()) return;
    
    QPushButton *btn = m_vmConnectButtons.value(vmId);
    if (btn) {
        btn->setEnabled(true);
        btn->setText(translate("connect_vm"));
    }
    m_vmStatusLabel->hide();
}

// 显示进程相关错误信息给用户
void LoginWindow::showProcessError(const QString &details)
{
    QMessageBox msgBox(this);
    msgBox.setIcon(QMessageBox::Warning);
    msgBox.setWindowTitle(translate("connect"));
    
    // 将详细信息截取前 500 字符显示在主文本区域（让用户不需要点"Show Details"）
    QString mainText = translate("connect_failed");
    if (!details.isEmpty()) {
        QString truncated = details.left(500);
...
void LoginWindow::onVmConnectClicked(const QString &vmId)
{
    ...
    m_connectingVmId = vmId;
    
    // 禁用连接按钮，取消"登录中..."提示
    QPushButton *connectBtn = m_vmConnectButtons.value(vmId);
    if (connectBtn) {
...
void LoginWindow::onVmRdpFileReply(QNetworkReply *reply, const QString &vmId)
{
    ...
    if (reply->error() != QNetworkReply::NoError) {
        ...
        restoreConnectButtonForVm(vmId);
        return;
    }
    ...
    loginRequest.setRawHeader(QByteArray("Authorization"), QByteArray("Bearer " + m_token.toUtf8()));
    loginRequest.setRawHeader(QByteArray("Content-Type"), QByteArray("application/json"));
    
    QNetworkReply *loginReply = m_networkManager->get(loginRequest);
...
void LoginWindow::onVmLoginReply(QNetworkReply *reply, const QString &vmId)
{
    ...
    if (json.contains("code") && json["code"].toInt() == 0) {
        ...
        // 查找 sdl3-freerdp 可执行文件
        QString freerdpPath = findFreeRdpBinary();
        if (freerdpPath.isEmpty()) {
            qDebug() << "onVmLoginReply: sdl3-freerdp binary not found";
            showProcessError(translate("connect_failed") + "\n\n" + 
                "sdl3-freerdp binary not found in app bundle.\n"
                "Please ensure sdl3-freerdp is included in Resources/");
            restoreConnectButtonForVm(vmId);
            return;
        }
        
        qDebug() << "onVmLoginReply: freerdpPath =" << freerdpPath;
        qDebug() << "onVmLoginReply: rdpFilePath =" << rdpFilePath;
        
        // 解析命令参数
        // 服务器返回的命令格式如: "sdl3-freerdp template.rdp"
        // 我们取出参数部分，替换 template.rdp 为完整路径
        QStringList args = command.split(" ", Qt::SkipEmptyParts);
        if (args.isEmpty()) {
            qDebug() << "onVmLoginReply: command is empty after split";
            QMessageBox::warning(this, translate("connect"), translate("connect_failed"));
            restoreConnectButtonForVm(vmId);
            return;
        }
        ...
        // 展开 shell 环境变量（如 $HOME）
        for (int i = 0; i < args.size(); ++i) {
            args[i].replace("$HOME", QDir::homePath());
        }
        
        qDebug() << "onVmLoginReply: arguments =" << args;
        
        // 获取当前屏幕分辨率，添加 /size 参数（如果不存在）
        {
            ...
            if (screen) {
                // 使用可用工作区域（已减去菜单栏和 Dock 栏），再减去窗口标题栏高度
                QSize workArea = screen->availableGeometry().size();
                int titleBarHeight = this->geometry().top() - this->frameGeometry().top();
                if (titleBarHeight > 0) {
                    workArea.setHeight(workArea.height() - titleBarHeight);
                }
                qreal dpr = screen->devicePixelRatio();
                QSize screenSize = QSize(workArea.width() * dpr, workArea.height() * dpr);
                bool hasSizeArg = false;
                for (const QString &arg : args) {
                    if (arg.startsWith("/size:") || arg.startsWith("/w:") || arg.startsWith("/h:")) {
                        hasSizeArg = true;
                        break;
                    }
                }
                if (!hasSizeArg) {
                    QString sizeArg = QString("/size:%1x%2").arg(screenSize.width()).arg(screenSize.height());
                    // 在 RDP 文件路径之后插入（FreeRDP 期望位置参数在标志参数之前）
                    args.insert(1, sizeArg);
                }
            }
        }
        ...
        connect(m_rdpProcess, &QProcess::started, this, [this]() {
            qDebug() << "onVmLoginReply: process started successfully (signal)";
        });
        ...
        // 设置工作目录为 freerdp 所在目录（确保相对路径的 dylib 能正确加载）
        QFileInfo fi(freerdpPath);
        m_rdpProcess->setWorkingDirectory(fi.absolutePath());
        
        qDebug() << "onVmLoginReply: starting process";
        qDebug() << "onVmLoginReply: workingDirectory =" << m_rdpProcess->workingDirectory();
        qDebug() << "onVmLoginReply: program =" << freerdpPath;
        qDebug() << "onVmLoginReply: arguments =" << args;
        
        // macOS APFS 的 com.apple.provenance 属性会导致 execve 被 AMFI 拦截 (exit 126)。
        // 直接使用 /usr/bin/arch -arm64 作为启动程序可以绕过此限制，
        // 因为 arch 是 Apple 签名的系统二进制，拥有绕过 AMFI 检查的权限。
        // sdl3-freerdp 的 @rpath 指向 @loader_path/../Frameworks，能自动找到 dylib，
        // 但需要避免继承 Qt 的 DYLD_FRAMEWORK_PATH（会干扰 dylib 查找）。
        QProcessEnvironment env = QProcessEnvironment::systemEnvironment();
        env.remove("DYLD_FRAMEWORK_PATH");
        env.remove("DYLD_LIBRARY_PATH");
        m_rdpProcess->setProcessEnvironment(env);
        
        // 工作目录设为二进制所在目录
        m_rdpProcess->setWorkingDirectory(fi.absolutePath());
        
        // 通过 arch -arm64 启动，绕过 provenance 限制
        QStringList archArgs;
        archArgs << "-arm64" << freerdpPath << args;
        
        qDebug() << "onVmLoginReply: launching via /usr/bin/arch";
        qDebug() << "onVmLoginReply: arch args =" << archArgs;
        qDebug() << "onVmLoginReply: workingDirectory =" << fi.absolutePath();
        
        m_rdpProcess->start("/usr/bin/arch", archArgs);
...
void LoginWindow::onRdpProcessReadyReadStandardError()
{
    ...
    m_rdpStderr += QString::fromUtf8(data);
...
void LoginWindow::onRdpProcessFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
    ...
    if (m_rdpProcess) {
        QString stdOut = m_rdpProcess->readAllStandardOutput();
        // 消费剩余的 stderr（可能 miss 掉一些，但 m_rdpStderr 已保存 readyRead 的数据）
        QString leftoverStderr = m_rdpProcess->readAllStandardError();
        if (!m_rdpStderr.isEmpty() || !leftoverStderr.isEmpty()) {
            m_rdpStderr += leftoverStderr;
        }
        
        if (!stdOut.isEmpty()) {
            qDebug() << "onRdpProcessFinished: stdout =" << stdOut;
        }
        if (!m_rdpStderr.isEmpty()) {
            qDebug() << "onRdpProcessFinished: stderr =" << m_rdpStderr;
        }
        
        // exit code 131 (128+3=SIGQUIT) = FreeRDP 用户关闭 RDP 窗口的正常退出
        if (exitCode == 131) {
            qDebug() << "onRdpProcessFinished: user closed RDP window (exit code 131)";
        }
        // 其他非 0 退出码才显示错误
        else if (exitCode != 0 || exitStatus != QProcess::NormalExit) {
            QString errorMsg;
            
            // exit code 126 = macOS AMFI/权限限制
            if (exitCode == 126) {
                errorMsg = "Process exit code: 126 (execve permission denied)\n\n"
                           "macOS security policies may be blocking execution.\n"
                           "Try: System Settings → Privacy & Security → scroll down\n"
                           "and allow the blocked application.";
            } else {
                errorMsg = "Process exit code: " + QString::number(exitCode) + "\n";
            }
            
            if (exitStatus != QProcess::NormalExit) {
                errorMsg += "Status: Crashed\n";
            }
            if (!m_rdpStderr.isEmpty()) {
                errorMsg += "\nError output:\n" + m_rdpStderr;
            }
            qDebug() << "onRdpProcessFinished: showing error";
            showProcessError(errorMsg);
        }
    }
    
    m_rdpStderr.clear();
    restoreConnectButtonForVm(m_connectingVmId);
    m_connectingVmId.clear();
}
...
void LoginWindow::onRdpProcessErrorOccurred(QProcess::ProcessError error)
{
    ...
    QString errorMsg;
    if (m_rdpProcess) {
        errorMsg = m_rdpProcess->errorString();
        qDebug() << "onRdpProcessErrorOccurred: errorString =" << errorMsg;
    } else {
        errorMsg = translate("connect_failed");
    }
